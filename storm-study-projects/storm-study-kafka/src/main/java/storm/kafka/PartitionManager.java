...
public class PartitionManager {
	...
	protected final FetchRequestBuilder builder;
	...
	public PartitionManager(DynamicPartitionConnections connections,
			...
			SpoutConfig spoutConfig, GlobalPartitionId id) {
		builder = new FetchRequestBuilder();
		_partition = id;
		...
		try {
			...
			LOG.warn("Error reading and/or parsing at ZkNode: "
					+ committedPath(), e);
		}

		if (!topologyInstanceId.equals(jsonTopologyId)
				&& spoutConfig.forceFromStart) {
			_committedTo = getOffsetsBefore(spoutConfig, id,
	...
	public long getOffsetsBefore(SpoutConfig spoutConfig, GlobalPartitionId id,
			long time) {

		return getOffsetsBefore(spoutConfig.topic, id.partition, time);
	}
	...
	public long getOffsetsBefore(String topic, int partition, long time) {
		...
		long[] startOffsets = _consumer.getOffsetsBefore(request).offsets(
				topic, partition);
	...
	public EmitState next(SpoutOutputCollector collector) {
		...
		while (true) {
			...
			Iterable<List<Object>> tups = _spoutConfig.scheme.deserialize(Utils
					.toByteArray(toEmit.msg.payload()));
			if (tups != null) {
	...
	private void fill() {
		...
		ByteBufferMessageSet msgs = _consumer.fetch(
				builder.addFetch(_spoutConfig.topic, _partition.partition,
						_emittedToOffset, _spoutConfig.fetchSizeBytes).build())
				.messageSet(_spoutConfig.topic, _partition.partition);
	...
	public long queryPartitionOffsetLatestTime() {
		return getOffsetsBefore(_spoutConfig.topic, _partition.partition,
				kafka.api.OffsetRequest.LatestTime());
	}
